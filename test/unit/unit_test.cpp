/*
 * Khulnasoft urlRequest test component
...
TEST_F(UrlRequestUnitTest, GetFileHttp)
{
    ...
    EXPECT_CALL(*request, setOption(optUrl, "http://www.khulnasoft.com/")).Times(1);
    ...
    GetRequest::builder(request).url("http://www.khulnasoft.com/").outputFile("/tmp/hello_world.html").execute();
}
...
TEST_F(UrlRequestUnitTest, GetFileWithUnixSocket)
{
    ...
    EXPECT_CALL(*request, setOption(optUnixSocketPath, "/tmp/khulnasoft-agent.sock")).Times(1);
    EXPECT_CALL(*request, setOption(optUrl, "http://www.khulnasoft.com/")).Times(1);
    EXPECT_CALL(*request, setOption(optCustomRequest, "GET")).Times(1);
    EXPECT_CALL(*request, setOption(optWriteData, SafeMatcherCast<void*>(_))).Times(1);
    EXPECT_CALL(*request, setOption(optWriteFunction, zero)).Times(1);
    ...
    GetRequest::builder(request)
        ...
        .unixSocketPath("/tmp/khulnasoft-agent.sock")
...
TEST_F(UrlRequestUnitTest, GetApiRequest)
{
    ...
    EXPECT_CALL(*request, setOption(optUserAgent, "Khulnasoft-Agent/1.0")).Times(1);
    ...
    GetRequest::builder(request)
        ...
        .userAgent("Khulnasoft-Agent/1.0")
        .certificate("cert.ca")
        .timeout(10)
        .execute();
}
...
TEST_F(UrlRequestUnitTest, PostApiRequestWithPostFields)
{
    ...
    EXPECT_CALL(*request, setOption(optCainfo, "cert.ca")).Times(1);
    EXPECT_CALL(*request, setOption(optTimeout, 10)).Times(1);
    EXPECT_CALL(*request, setOption(optPostFields, R"({"name":"khulnasoft"})")).Times(1);
    EXPECT_CALL(*request, setOption(optPostFieldSize, std::string(R"({"name":"khulnasoft"})").length())).Times(1);
    EXPECT_CALL(*request, setOption(optVerifyPeer, true)).Times(1);
    EXPECT_CALL(*request, execute()).Times(1);
    ...
    PostRequest::builder(request)
        ...
        .certificate("cert.ca")
        .timeout(10)
        .postData(R"({"name":"khulnasoft"})"_json)
        .execute();
}
...
TEST_F(UrlRequestUnitTest, HttpsCertExists)
{
    ...
    EXPECT_CALL(*request, setOption(optUrl, "https://www.khulnasoft.com/")).Times(1);
    ...
    EXPECT_NO_THROW(getRequest.url("https://www.khulnasoft.com/").execute());
}
...
TEST_F(UrlRequestUnitTest, HttpsNoCertNotExists)
{
    ...
    EXPECT_NO_THROW(getRequestNoCert.url("https://www.khulnasoft.com/").execute());
}
